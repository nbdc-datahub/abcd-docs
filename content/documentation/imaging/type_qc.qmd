...
# Recommended imaging inclusion {#recommended-imaging-inclusion}
...
## dMRI post processing QC {#dmri-post-processing-qc}
...
## fMRI post processing QC {#fmri-post-processing-qc}
...
# Automated post processing QC {#automated-post-processing-qc}
...
Our manual quality control process involves manual examination of brain images from a subset of participant-events for each modality, with ratings for each dataset according to pre-defined quality criteria such as brain cutoff, residual distortion, or registration to T~1~. Such measures are described above (see [*FreeSurfer QC*](d#freesurfer-qc), [*sMRI T~2~w Post Processing QC*](#smri-t2w-post-processing-qc), [*dMRI Post Processing QC*](#dmri-post-processing-qc), and [*fMRI Post Processing QC)*](#fmri-post-processing-qc). Because an exhaustive manual review of every scan is not practical, we deploy statistical learning/AI guided sub-sampling methods where we generate automated metrics for an array of quality control issues (see [*Automated Post Processing QC*](#automated-post-processing-qc)) and sub-select participant-events to be sent for a given modality to manual review based on their quality measure scores. The automatic selection priority scores generally indicate how likely a dataset of a given type is to contain identifiable data quality issues such as a residual distortion or brain cutoff.
