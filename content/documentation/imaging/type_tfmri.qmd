...
# Domain overview
```{r}
render_table_info_imaging("sub_domain == 'Task fMRI'")
```

<!-- [Download table as a pdf](../../assets/files/documentation/imaging/6_1_mr_tfmri.pdf){style="display: block; text-align: right; margin-top: 4px;"} -->
...
::: {.callout-caution collapse="true" title="Release notes: `mr_y_tfmri`"}
**6.0**

-   [**GE - Timing triggers**](../release_notes/6_0.qmd#event-timing-offset-for-ge-scanners): There was previously a discrepancy in how stimulus times were modeled relative to the end of the calibration/dummy volumes in the image acquisition that affects GE scanners.

Additionally, in a relatively small, though substantial, subset of task fMRI acquisitions collected on GE scanners (~9% of runs), the time between the 1st and 16th trigger pulse sent from the scanner does not match the expected 12 seconds.
:::
...
**Key reference:** `r full_cite("casey2018")`
...
- `r full_cite("chang1992")`
- `r full_cite("morgan2004")`
