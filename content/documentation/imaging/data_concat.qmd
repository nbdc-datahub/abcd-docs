...
A detailed description of how the ABCD3 atlas was generated and registration steps can be found in @pecheva2022. The ABCD3 atlas files are available to download from https://zenodo.org/records/17258390.
...
|                                      | runs: r01, r02                                                                        |
+--------------------------------------+---------------------------------------------------------------------------------------+
| `mid_{condition}_{stat}.mat`         | task: mid (Monetary Incentive Delay)\                                                 |
|                                      | conditions: see below                                                                 |
| e.g., `mid_sra_beta.mat`             |                                                                                       |
|                                      | stats: beta                                                                           |
|                                      |                                                                                       |
|                                      | runs: average of r01, r02                                                             |
+--------------------------------------+---------------------------------------------------------------------------------------+
| `nback_{condition}_{stat}.mat`       | task: nback (Emotional n-Back)                                                        |
|                                      |                                                                                       |
| e.g., `nback_2b_beta.mat`            | conditions: see below                                                                 |
|                                      |                                                                                       |
|                                      | stats: beta                                                                           |
|                                      |                                                                                       |
|                                      | runs: average of r01, r02                                                             |
+--------------------------------------+---------------------------------------------------------------------------------------+
| `sst_{condition}_{stat}.mat`         | task: sst (Stop Signal Task)                                                          |
|                                      |                                                                                       |
| e.g., `sst_cg_beta.mat`              | conditions: see below                                                                 |
|                                      |                                                                                       |
|                                      | stats: beta                                                                           |
|                                      |                                                                                       |
|                                      | runs: average of r01, r02                                                             |
...
-   File name patterns above with `{condition}` and `{stat}`, lacking "r01" or "r02", represent the average of available runs.
-   Outlier beta values (i.e., outside the mean ± 3 x standard deviation) are present for some particular voxels, runs, conditions, and/or participant-events. See “Task fMRI Outliers” in [Task-Based fMRI Data Documentation](type_tfmri.qmd) for more information and guidance.
...
**Contents of `{task}\_{condition}\_{stat}\_{run}.mat` or `{task}\_{condition}\_{stat}.mat`**
...
|                                             | hemispheres: lh, rh                   |
+---------------------------------------------+---------------------------------------+
| `mid_{condition}_{stat}_{hemi}.mat`         | task: mid (Monetary Incentive Delay)\ |
|                                             | conditions: see below                 |
| e.g., `mid_sra_beta_lh.mat`                 |                                       |
|                                             | stats: beta, sem                      |
| ```                                         |                                       |
|     `mid_sra_sem_lh.mat`                    | runs: average of r01, r02             |
| ```                                         |                                       |
|                                             | hemispheres: lh, rh                   |
+---------------------------------------------+---------------------------------------+
| `nback_{condition}_{stat}_{hemi}.mat`       | task: nback (Emotional n-Back)        |
|                                             |                                       |
| e.g., `nback_2b_beta_lh.mat`                | conditions: see below                 |
|                                             |                                       |
| ```                                         | stats: beta, sem                      |
|     `nback_2b_sem_lh.mat`                   |                                       |
| ```                                         | runs: average of r01, r02             |
...
| `sst_{condition}_{stat}_{run}_{hemi}.mat`   | task: sst (Stop Signal Task)          |
|                                             |                                       |
| e.g., `sst_cg_beta_lh.mat`                  | conditions: see below                 |
|                                             |                                       |
| ```                                         | stats: beta, sem                      |
|     `sst_cg_sem_lh.mat`                     |                                       |
| ```                                         | runs: average of r01, r02             |
...
-   File name patterns above with `{condition}`, `{stat}`, and `{hemi}`, lacking "r01" or "r02",\`represent the average of available runs.
-   Outlier beta values (i.e., outside the mean ± 3 x standard deviation) are present for some particular vertices, runs, conditions, and/or participant-events. See “Task fMRI Outliers” in [Task-Based fMRI Data Documentation](type_tfmri.qmd) for more information and guidance.
...
**Contents of `{task}\_{condition}\_{stat}\_{run}\_hemi}.mat` or `{task}\_{condition}\_{stat}\_hemi}.mat`**
...
-   File names with `trim5` or `trim10` indicate correlations derived from time series data “trimmed” to either 5 or 10 minutes of low-motion (FD \< 0.2 mm) time points. Time points were randomly selected from available time points. Excess time points were discarded. Correlations for visits with fewer available time points were set to NaN (see [Responsible use consideration: Head motion](type_rsfmri.qmd#use-rsfmri) in Resting-State fMRI Data Documentation).\
...
+--------------------+-----------------------------------------------------------------------------------------------------------------------------+
| `nvolvec`          | number of frames after skipTR                                                                                               |
|                    |                                                                                                                             |
|                    | vector with size = \[ndirs,1\]                                                                                              |
+--------------------+-----------------------------------------------------------------------------------------------------------------------------+
| `nvolfdsubvec`     | number of frames with fd \< censor_thresh                                                                                   |
|                    |                                                                                                                             |
|                    | vector with size = \[ndirs,1\]                                                                                              |
+--------------------+-----------------------------------------------------------------------------------------------------------------------------+
| `nvolfdsubcontvec` | number of frames with fd \< censor_thresh and at least 5 contiguous uncensored frames                                       |
|                    |                                                                                                                             |
|                    | vector with size = \[ndirs,1\]                                                                                              |
+--------------------+-----------------------------------------------------------------------------------------------------------------------------+
| `ntpointvec`       | number of valid time points                                                                                                 |
|                    |                                                                                                                             |
|                    | i.e., those retained after motion censoring number of frames after censoring (fd, contig, and outlier)                      |
|                    |                                                                                                                             |
|                    | vector with size = \[ndirs,1\]                                                                                              |
+--------------------+-----------------------------------------------------------------------------------------------------------------------------+
| `meanfdvec`        | average framewise displacement                                                                                              |
|                    |                                                                                                                             |
|                    | vector with size = \[ndirs,1\]                                                                                              |
+--------------------+-----------------------------------------------------------------------------------------------------------------------------+
| `meanfdmat`        | average of absolute values of the framewise differences in 6-axis motion estimates with this order: dx, dy, dz, rx, ry, rz  |
|                    |                                                                                                                             |
|                    | also called: dL, dP, dS, pitch, yaw, roll rotation                                                                          |
|                    |                                                                                                                             |
|                    | components (rx, ry, and rz) are scaled to units of mm assuming a brain with \~50 mm radius                                  |
|                    |                                                                                                                             |
|                    | vector with size = \[ndirs,6\]                                                                                              |
+--------------------+-----------------------------------------------------------------------------------------------------------------------------+
| `meanrotvec`       | average rotation (rx, ry, rz)                                                                                               |
|                    |                                                                                                                             |
|                    | sum of abs(rx) + abs(ry) + abs(rz)                                                                                          |
|                    |                                                                                                                             |
|                    | vector with size = \[ndirs,1\]                                                                                              |
+--------------------+-----------------------------------------------------------------------------------------------------------------------------+
| `meantransvec`     | average translation (dx, dy, dz)                                                                                            |
|                    |                                                                                                                             |
|                    | sum of abs(dx) + abs(dy) + abs(dz)                                                                                          |
|                    |                                                                                                                             |
|                    | vector with size = \[ndirs,1\]                                                                                              |
+--------------------+-----------------------------------------------------------------------------------------------------------------------------+
| `maxrotvec`        | maximum rotation                                                                                                            |
|                    |                                                                                                                             |
|                    | vector with size = \[ndirs,1\]                                                                                              |
+--------------------+-----------------------------------------------------------------------------------------------------------------------------+
| `maxtransvec`      | maximum translation                                                                                                         |
|                    |                                                                                                                             |
|                    | vector with size = \[ndirs,1\]                                                                                              |
+--------------------+-----------------------------------------------------------------------------------------------------------------------------+
| `maxfdvec`         | maximum framewise displacement                                                                                              |
|                    |                                                                                                                             |
|                    | vector with size = \[ndirs,1\]                                                                                              |
+--------------------+-----------------------------------------------------------------------------------------------------------------------------+
| `meanfdcvec`       | average framewise displacement                                                                                              |
|                    |                                                                                                                             |
|                    | across valid time points                                                                                                    |
|                    |                                                                                                                             |
|                    | vector with size = \[ndirs,1\]                                                                                              |
+--------------------+-----------------------------------------------------------------------------------------------------------------------------+
| `meanfdcmat`       | average framewise displacement matrix                                                                                       |
|                    |                                                                                                                             |
|                    | across valid time points                                                                                                    |
|                    |                                                                                                                             |
|                    | vector with size = \[ndirs,6\]                                                                                              |
+--------------------+-----------------------------------------------------------------------------------------------------------------------------+
| `meanrotcvec`      | average rotation (rx, ry, rz)                                                                                               |
|                    |                                                                                                                             |
|                    | across valid time points                                                                                                    |
|                    |                                                                                                                             |
|                    | vector with size = \[ndirs,1\]                                                                                              |
+--------------------+-----------------------------------------------------------------------------------------------------------------------------+
| `meantranscvec`    | average translation (dx, dy, dz)                                                                                            |
|                    |                                                                                                                             |
|                    | across valid time points                                                                                                    |
|                    |                                                                                                                             |
|                    | vector with size = \[ndirs,1\]                                                                                              |
+--------------------+-----------------------------------------------------------------------------------------------------------------------------+
| `maxrotcvec`       | maximum rotation                                                                                                            |
|                    |                                                                                                                             |
|                    | across valid time points                                                                                                    |
|                    |                                                                                                                             |
|                    | vector with size = \[ndirs,1\]                                                                                              |
+--------------------+-----------------------------------------------------------------------------------------------------------------------------+
| `maxtranscvec`     | maximum translation                                                                                                         |
|                    |                                                                                                                             |
|                    | across valid time points                                                                                                    |
|                    |                                                                                                                             |
|                    | vector with size = \[ndirs,1\]                                                                                              |
+--------------------+-----------------------------------------------------------------------------------------------------------------------------+
| `maxfdcvec`        | maximum framewise displacement                                                                                              |
|                    |                                                                                                                             |
|                    | across valid time points                                                                                                    |
|                    |                                                                                                                             |
|                    | vector with size= \[ndirs, 1\]                                                                                              |
...
-   File names with “gpnet” indicate correlations within or between networks defined by the Gordon parcel “communities”.
-   File names with “trim5” indicate correlations derived from time series data “trimmed” to 5 minutes of low-motion (FD \< 0.2 mm) time points. Time points were randomly selected from available time points. Excess time points were discarded. Correlations for visits with fewer available time points were set to NaN (see [Responsible use considerations: Head Motion](type_rsfmri.qmd) in Resting-State fMRI Data Documentation).
