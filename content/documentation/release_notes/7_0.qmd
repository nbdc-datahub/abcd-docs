...
The ABCD 7.0 data release also includes data from associated substudies: COVID-19, Endocannabinoid, Hurricane Irma, MR Spectroscopy, Social Development, and the newly added RECOVER, SIPS, and Baby Teeth substudies. Some of these substudy assessments are conducted during the same visits as the main study, while others have independent event structures.

The following table shows the number of participants with data per substudy event (substudies that don’t have their own event structure are not listed):
...
A data compilation error in the 7.0 release caused `mh_y_resil` to only include data for ses-06A and ses-07A. Prior releases (6.1 and 6.0) have the prior timepoints. This issue is anticipated to be fixed in the 7.1 release.
...
New dynamic and static substance use (SU) summary scores (`su_y_dyn`, `su_y_stc`) have been introduced in the 7.0 Release to harmonize substance use indicators across multiple measures (Substance Use Interview, TLFB, and Mid-Year Phone Interview). Details on these new variables can be found in the SU domain documentation for both the `r link_table("su_y_dyn", dest = "docs", name = "label", type = "md")` and `r link_table("su_y_stc", dest = "docs", name = "label", type = "md")` tables.
...
###  Registration quality issue for concatenated voxelwise data 

A bug has been identified in the registration pipeline used to produce the ABCD 7.0 concatenated voxelwise data. An outdated version of the target image for the 11th channel of the ABCD3 atlas was used in the multimodal registration. The 11th channel in the multimodal registration is a grey matter segmentation mask. The outdated target image did not include cerebellar gray matter, while participants' input (moving) image did. This mismatch between the target and moving images resulted in systematically degrading alignment quality for that channel across all participants. Mean correlation between participant scan and atlas template for the 11th channel decreased by ~0.23 compared to release 6.0. Overall, mean correlation between participants' scans and the atlas (across all 11 channels) was reduced by 0.0108. Any QC filter based on mean correlation to atlas will exclude a greater proportion of participants comapred to release 6.0. 
...
### COVID Symptoms Surveys: Known Variable Naming Issues

During QC review of the COVID Symptoms survey tables (`recover_p_ps` youth-report and `recover_p_pscg` caregiver-report), a systematic variable naming error was identified affecting 346 variables. The underlying response data are unaffected. These will be corrected in the 7.1 patch release.

The misnaming occurred in the same pattern in both surveys, users referencing these variables should apply the following rename mapping:

| Current name (incorrect) | Correct name | Survey | n |
|--------------------------|--------------|--------|:-:|
| `recover_p_ps__infect__2wk_001–089` | `recover_p_ps__curr_001–089` | Youth-report | 89 |
| `recover_p_ps__pia_001–089` | `recover_p_ps__infect__2wk_001–089` | Youth-report | 89 |
| `recover_p_pscg__infect__ll_001–084` | `recover_p_pscg__curr_001–084` | Caregiver-report | 84 |
| `recover_p_pscg__ilt_001–084` | `recover_p_pscg__infect__ll_001–084` | Caregiver-report | 84 |

Researchers working in R can apply the corrections using the following code:

```r
rename_recover_covid_symptoms <- function(data) {
  names(data) <- names(data) |>
    stringr::str_replace(
      "recover_p_ps__infect__2wk_",
      "recover_p_ps__curr_"
    ) |>
    stringr::str_replace(
      "recover_p_ps__pia_",
    ...
    stringr::str_replace(
      "recover_p_pscg__infect__ll_",
      "recover_p_pscg__curr_"
    ) |>
    stringr::str_replace(
      "recover_p_pscg__ilt_",
      "recover_p_pscg__infect__ll_"
    )
  data
}
...
data <- rename_recover_covid_symptoms(data)
```

Additionally, variable labels for `recover_p_pscg__noinfect__ll_001–084` currently read "Not long-lasting: \<symptom\>" and will be corrected to "Intermediate/Long-lasting (No infection history): \<symptom\>" in the next data dictionary update.
