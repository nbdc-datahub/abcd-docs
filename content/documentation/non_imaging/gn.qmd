...
-   [**Genetic Relatedness Matrix**](../release_notes/6_1.qmd#genetic-relatedness-matrix): A square, MATLAB file formatted (.mat) version of the Genetic Relatedness Matrix has been added to the file based data.

**6.0**

-   [**File naming & inclusion**](../release_notes/6_0.qmd#genetics): Consistent with the rest of the 6.0 data release, the `NDAR_INV` prefix has been removed from all subject identifiers in files. Four individuals were also removed from files due to withdrawal of consent or familial genetic relatedness inconsistency.
-   [**Variant call files**](../release_notes/6_0.qmd#topmed-imputed-data-shared-as-vcfs): TOPMED imputed data is now shared as VCFs (Variant Call Files) instead of as PLINK files; this facilitates the inclusion of imputation INFO scores which indicate the quality of imputation and enables the inclusion of multiallelic variants.
:::
...
-   [**Missing genotypes**](../release_notes/6_0.qmd#genetically-derived-family-and-birth-ids): In the 6.0 data release, 200 individuals have still not been genotyped from the full enrolled sample and so `gn_y_genrel_id__fam` and `gn_y_genrel_id__birth` are not defined for these individuals.
-   [**Family relatedness**](../release_notes/6_0.qmd#genetically-derived-family-and-birth-ids): In previous data releases, family relatedness was captured by `rel_family_id`, this variable is now named `gn_y_genrel_id__fam` in the genetics table and crosslisted as `ab_g_stc__design_id__fam__gen` in the `ab_g_stc` table.
:::
