...
└── concatenated
    └── genetics
        ├── genotype_microarray
        │   ├── imputed
        │   │   ├── chr*_dose.<vcf.gz|vcf.gz.tbi>
        │   │   └── qcreport.html 
        │   ├── genesis
        │   │   ├── pcair_weights.tsv
        │   │   ├── pcrelate_relatedness.<grm.bin|grm.id|grm.N.bin|tsv>
        ...
        └── sequencing
            ├── gene_burden_mat
            │   └── genome_wide_gene_burden_matrix.<genemasks.txt|mtx>
            ├── metadata
            │   ├── picard_metrics.tsv
            │   └── sample_info.tsv
            ├── snv_indel
            │   └── population_vcf
            │      └── abcd_cohort_chr*_block*.<vcf.gz|vcf.gz.tbi>
            └── supportfiles
                ├── GRCh38.<no_alt_analysis_set.fa|no_alt_analysis_set.fa.fai>
                ├── low_complexity_regions.tsv
                └── pvcf_blocks.txt
...
For extraction of single genetic variants, users can use tools like [bed-reader](https://github.com/fastlmm/bed-reader) for python and [snpStats](https://rdrr.io/bioc/snpStats/man/read.plink.html) for R to parse plink bed files described below. Methods such as [PRS-csx](https://github.com/getian107/PRScsx) have been developed to generate polygenic scores in samples with the high degree of genetic diversity found in ABCD. [PRScs](https://github.com/getian107/PRScs) shows comparable performance with PRS-csx in ABCD data, whilst requiring fewer analysis steps (e.g. cross validation)(Ahern, J et. al., Behavior Genetics, 2023). The performance of polygenic scores varies as genomic distance from the training sample increases (Ding et. al., 2023). 

::: {#use-micro .callout-important collapse="true" title="Responsible use consideration: Micro Array Based Data"}
Due to the variation in performance of polygenic scores, conducting stratified analysis of individuals that share similar continental ancestry is currently considered best practice, although this is an evolving field with new approaches constantly being developed. Tools such as [ADMIXTURE](https://dalexander.github.io/admixture/) can generate genetic factors to enable this type of analysis.
:::
