...
<!-- [Download table as a pdf](../../assets/files/documentation/non_imaging/6_1_ph.pdf){style="display: block; text-align: right; margin-top: 4px;"} -->

Learn about the Physical Health domain by reviewing the below references: 

- `r full_cite("uban2018")`
- `r full_cite("herting2021")`
- `r full_cite("palmer2021")`
...
::: {.callout-caution collapse="true" title="Release notes: `ph_y_mctq`"}
**7.0**

-   [**Algorithm updated**](../release_notes/7_0.qmd#mctq-score-update): The algorithm for the MCTQ free day and school day count items has been updated such that if participants endorse they no longer go to school or work, the free day count will = `7` and the school/work day count will = `0` rather than missing as they were in prior releases. This should help improve missingness in other scores that were dependent on these fields having values.

**6.1** 

-   [**Algorithm updated**](../release_notes/6_1.qmd#munich-chronotype-questionnaire)
:::

**Key reference:** `r full_cite("zavada2005")`

## Medications Inventory (Youth) {#ph_y_meds}

`r table_header("ph_y_meds")`

**Measure description:** Medications youth participant has taken in the last two weeks. The medication questionnaire was drawn from the [PhenX Toolkit](https://www.phenxtoolkit.org/search/results?searchTerm=medications&searchtype=smartsearch). Drug names, dosage, etc. used the [RxNorm](https://www.nlm.nih.gov/research/umls/rxnorm/index.html) database available from the NIH National Library of Medicine.

**Modifications since initial administration:** We added a question to the Youth Medication Inventory on May 2024 asking the following: "In the past year, did you take cannabis (with THC), ketamine, psilocybin, or MDMA for medical reasons with your parent’s or a doctor's permission?"

**Notes:**

::: {.callout-caution collapse="true" title="Release notes: `ph_y_meds`"}
**7.0**

-   [**Summary scores**](../release_notes/7_0.qmd#medications-inventory-&-developmental-history-summary-scores): Summary scores have been developed for the Medications Inventory (`ph_y_meds`) table.
:::


## Youth Risk Behavior Survey---Physical Activity {#ph_y_pa}

`r table_header("ph_y_pa")`

**Measure description:** A brief assessment of physical exercise.

**Key references:** 

- `r full_cite("dolsen2019")`
- `r full_cite("hunsberger2015")`

## Pubertal Development Scale & Menstrual Cycle Survey History (Youth) {#ph_y_pds}

`r table_header("ph_y_pds")`

**Measure description:** Pubertal stage and menstrual phase (for postmenarcheal girls).

**Modifications since initial administration:** 

Five items on hormonal contraception and menstrual pain were added at 3-year follow-up for girls.

Due to a change in branching logic, some menstrual cycle items [`ph_y_pds__f_002__07`, `ph_y_pds__f_002__08`, `ph_y_pds__f_002__09`, `ph_y_pds__f_002__10`] were not administered in Year 4 until April 12, 2022, leading to missingness on these items for participants who completed Year 4 visit prior to this date.
...
- `r full_cite("petersen1988")`
- `r full_cite("cheng2021")`
- `r full_cite("herting2021")`

## Pain Questionnaire {#ph_y_pq}

`r table_header("ph_y_pq")`
...
::: {.callout-caution collapse="true" title="Release notes: `ph_y_pq`"}
**7.0** 

-   [**Pain questionnaire**](../release_notes/7_0.qmd#pain-questionnaire): Previously, variable names were swapped for four items in the `ph_y_pq` table. The left and right variable names, but not labels, were reversed for the palm and top of foot items in this measure. This has been fixed in 7.0 Release data.

**6.0**

-   [**Variable naming error**](../release_notes/6_0.qmd#variable-naming-error-for-pain-questionnaire)
:::

**Key reference:** `r full_cite("luntamo2012")`

## Respiratory Functioning {#ph_y_rq}

`r table_header("ph_y_rq")`
...
- `r full_cite("gillman2018")`
- `r full_cite("asher1995")`

## Sports and Activities Involvement Questionnaire (Youth) {#ph_y_saiq}

`r table_header("ph_y_saiq")`
...
**Notes and special considerations:**
Data collection of Sexual Behavior, Orientation, and Communication was paused during 6 Year and 7 Year data collection, resulting in smaller table totals for ph_y_sex and ph_p_sex at ses-06A and ses-07A. 
...
::: {.callout-caution collapse="true" title="Release notes: `ph_y_sex`"}
**6.0**

-   [**New domain classification**](../release_notes/6_0.qmd#sexual-behavior-orientation-and-communication): Variables related to sexual behavior, orientation, and communication are now available in the `Physical Health` domain, under the `Sex` subdomain.
:::


**Key references:**

- `r full_cite("potter2022")`
- `r full_cite("sales2008")`
- `r full_cite("windle2004")`
...
::: {#use-ph_y_anthr .callout-important collapse="false" title="Resoponsible use consideration: Anthropometric Data"}
...
::: {.callout-caution collapse="true" title="Release notes: `ph_p_anthr`"}
**7.0** 

-   [**Height**](../release_notes/7_0.qmd#new-derived-height-scores): New derived items were added to the `ph_p_anthr` table to combine feet and inches into a single height item for both mother and father. 

**6.1** 

-   [**Height**](../release_notes/6_1.qmd#anthropometrics)
...
- `r full_cite("cdc2021")`
- `r full_cite("adise2024")`
- `r full_cite("berg2023")`

## Blood Analysis {#ph_y_bld}

`r table_header("ph_y_bld")`
...
::: {.callout-warning collapse="true" title="Data consideration: Blood analysis" #data-blood}
...
Information on genetics results from blood can be found in the [Genetics documentation](gn.qmd#smokescreen-binarized-plink-files-and-batch-info).

::: {.callout-caution collapse="true" title="Release notes: `ph_y_bld`"}
**7.0** 

- [**Blood results**](../release_notes/7_0.qmd#blood-results): Data errors in the blood results were discovered in the 6.0 dataset. All values that were correctable for 7.0 were fixed, but some outliers still remain.
...
::: {.callout-caution collapse="true" title="Release notes: `ph_y_bp`"}
**7.0** 
- [**Blood pressure**](../release_notes/7_0.qmd#blood-pressure): Previously, there were some errors with the summary scores counting invalid inputs like 0, and there were some outliers in the data. These issues have been addressed in 7.0 Release data

**6.0**

- [**Data outliers**](../release_notes/6_0.qmd#blood-pressure)
:::

## Saliva Analysis---Hormones {#ph_y_phs}

`r table_header("ph_y_phs")`
...
**Key reference:** `r full_cite("dolsen2019")`

# Parent tables

## Anthropometrics (Parent) {#ph_p_anthr}

`r table_header("ph_p_anthr")`
...
**Key reference:** `r full_cite("hunsberger2015")`

## Breastfeeding Questionnaire {#ph_p_bfq}

`r table_header("ph_p_bfq")`
...
**Key reference:** `r full_cite("kessler2009")`

## Child Nutrition Assessment {#ph_p_cna}

`r table_header("ph_p_cna")`
...
**Key reference:** `r full_cite("morris2015")`

## Annual ABCD COVID Survey (Parent) {#ph_p_covid}

`r table_header("ph_p_covid")`
...
ABCD participated in the RECOVER study; additional COVID-related survey items were completed by ABCD participants who opted to participate in the RECOVER study; those data are located within the [RECOVER dataset](https://recovercovid.org/data). See [RECOVER data documentation](../substudies/recover.qmd). 

::: {.callout-caution collapse="true" title="Release notes: `ph_p_covid`"}
**7.0**

-   [**Summary scores**](../release_notes/7_0.qmd#covid-survey): In the table `ph_p_covid` it was discovered that there were many date entries throughout the form that were implausible, such as dates from before or after the COVID pandemic. These have been corrected when possible or removed for the 7.0 release. 
:::
...
**Modifications since initial administration:** In the 4-year follow-up, responses from baseline were pre-populated into the questionnaire and the research assistant asked the parent to update where necessary. The form was shortened to only re-ask some of the items (e.g. alcohol/tobacco/marijuana use, age of first word, etc.) Added new pre-eclampsia/eclampsia/toxemia follow-up questions.

::: {.callout-caution collapse="true" title="Release notes: `ph_p_dhx`"}
**7.0**

-   [**Summary scores**](../release_notes/7_0.qmd#medications-inventory-&-developmental-history-summary-scores): Summary scores have been developed for the medications information collected in the (`ph_p_dhx`) table.

-   [**New derived birthweight item**](../release_notes/7_0.qmd#new-derived-birthweight-item): A new derived item was added to combine pounds and ounces into a single birthweight item.
:::

::: {.callout-warning collapse="true" title="Data consideration: Birthweight" #data-birthweight}
The birth weight data were parent-reported so there are outliers, and should be noted and interpreted as such.
:::

**Key references:**

- `r full_cite("kessler2009")`
- `r full_cite("merikangas2009")`

## International Physical Activity Questionnaire {#ph_p_ipaq}

`r table_header("ph_p_ipaq")`
...
**Key reference:** `r full_cite("booth2000")`

## Medications Inventory (Parent) {#ph_p_meds}

`r table_header("ph_p_meds")`
...
::: {.callout-caution collapse="true" title="Release notes: `ph_p_meds`"}
**7.0**

-   [**Summary scores**](../release_notes/7_0.qmd#medications-inventory-&-developmental-history-summary-scores): Summary scores have been developed for the Medications Inventory (`ph_p_meds`) table.

**6.1** 

-   [**Medications inventory**](../release_notes/6_1.qmd#medications-inventory): The variables `ph_p_meds_001__v01` and `ph_p_meds_001__v02` were collapsed into a single variable in the dataset due to congruency of the items.
...
**Key reference:** `r full_cite("todd2003")`



## Ohio State Traumatic Brain Injury Screen (Parent)  {#ph_p_otbi}

`r table_header("ph_p_otbi")`
...
- Year 3: Parents asked all questions, as well as reading and music questions (worded differently from the way that reading and music questions were asked in baseline); youth asked questions pertaining to reading and music questions only
- Year 4: Parents asked all questions EXCEPT reading and music questions; youth asked reading and music questions only
- Year 5, 6 and 7: Parents asked NO questions; youth asked ALL questions

**Key reference:** `r full_cite("huppertz2016")`

## Sleep Disturbance Scale for Children {#ph_p_sds}

`r table_header("ph_p_sds")`

**Measure description:** Parent survey of youth sleep and sleep disorders.


::: {.callout-caution collapse="true" title="Release notes: `ph_p_sds`"}
**6.1**

-   [**SDS summary scores added**](../release_notes/6_1.qmd#sleep-disturbance-scale-for-children): Sleep Disturbance Scale for Children (SDS) summary scores have been added to the `ph_p_sds` table.

**6.0**

-   [**Missing dates**](../release_notes/6_0.qmd#missing-dates-for-sleep-disturbance-scale-for-children): Some dates are missing for the `ph_p_sds` data (mostly at baseline and 1 year followup). 

-   [**SDS summary score**](../release_notes/6_0.qmd#sleep-disturbance-scale-for-children-sds-summary-scores)
:::
...
- `r full_cite("bruni1996")`
- `r full_cite("ferreira2009")`



## Sexual Orientation & Communication {#ph_p_sex}

`r table_header("ph_p_sex")`
...
**Notes and special considerations:** Data collection of Sexual Behavior, Orientation, and Communication was paused during 6 Year and 7 Year data collection, resulting in smaller table totals for ph_y_sex and ph_p_sex at ses-06A and ses-07A. 

If using these categorical variables for sexual orientation, careful consideration should go into whether and how to use them. For example, researchers should avoid the assumption that a "no" response to "Is your child gay?" is the same as an endorsement of their child being heterosexual. For sexual orientation, it is best practice to use self-report. If parent- or informant-report is used in the absence of self-report, it should be with acknowledgement of the limitations.
...
::: {.callout-caution collapse="true" title="Release notes: `ph_p_sex`"}
**6.0**

- [**New domain classification**](../release_notes/6_0.qmd#sexual-behavior-orientation-and-communication): In the 6.0 data release, variables related to sexual behavior, orientation and communication are available in the `Physical Health` domain, under the `Sex` subdomain.
:::

**Key reference:** `r full_cite("sales2008")`

## Baby Teeth Collection {#ph_p_teeth}

`r table_header("ph_p_teeth")`

**Measure description:** Beginning at the initial (baseline) appointment of ABCD, researchers collected naturally shed deciduous (“baby”) teeth from participants, the rationale, procedure, and analysis of which have been described in detail (@uban2018). So far 448 teeth have been analyzed and details on this data are available on the [baby teeth substudy page](../substudies/baby_teeth.qmd).

::: {.callout-caution collapse="true" title="Release notes: `Baby Teeth`"}
**7.0** 

-   [**Baby teeth**](../release_notes/7_0.qmd#baby-teeth): The data from 448 teeth are provided in the 7.0 Release data.
:::

**Key reference:** `r full_cite("uban2018")`
