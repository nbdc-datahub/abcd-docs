...
The algorithm in ABCDscores for the `ph_y_mctq` scores ending with `__sleep__mid__24_t` was updated.
...
-   [**Variable naming error**](../release_notes/6_0.qmd#variable-naming-error-for-pain-questionnaire): Variable names were swapped for four items in the `ph_y_pq` table. The left and right variable names, but not labels, were reversed for the palm `ph_y_pq__hand__plml_001`/`ph_y_pq__hand__plmr_001` and top of foot `ph_y_pq__foot__topl_001`/`ph_y_pq__foot__topr_001` items in this measure.
:::
...
::: {.callout-caution collapse="true" title="Release notes: `ph_p_anthr__height`"}
**6.1** 

-   [**Height**](../release_notes/6_1.qmd#anthropometrics): 
The levels values in the `ph_p_anthr__height` were remapped such that if a participant selected the response option "5 feet" that value is now coded as a `5` in the dataset whereas in 6.0 the values did not match.
...
- [**Data outliers**](../release_notes/6_0.qmd#neurocoginition-known-issues): There are some errors with the summary scores counting invalid inputs like 0, and there are some outliers in the data. 
:::
