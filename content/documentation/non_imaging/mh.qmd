...
There is an error with the summary score `mh_y_bisbas__bis_sum` computation as well as data for the variable `mh_y_bisbas__bis_005`.
:::
 ...
 **6.1**
 ...
 -   [**Suicidality module**](../release_notes/6_1.qmd#ksads-suicidality-youth): Researchers should not use any items about past suicidal ideation/behavior reports (any items starting with the prefix `mh_y_ksads__suic__past`) from the `mh_y_ksads__suic` table from the 5-year follow up event and after.
...
-   [**Borderline personality disorder**](../release_notes/6_0.qmd#ksads-borderline-personality-disorder): The variable `mh_y_ksads__bpd__irrit__expl__pres_sx__v01` is not a true symptom score and thus should be ignored. Data users interested in this symptom data should only use the `mh_y_ksads__bpd__irrit__expl__pres_sx` variable.
:::
...
Some cases are coded as 1 (Present) for lifetime suicidality, but these should all be coded as 555 (Not Administered). This correction will be implemented in the 6.1 patch release.
...
-   [**Life events summary score error**](../release_notes/6_0.qmd#life-events-summary-scores): The Life Events summary scores should not be used, because there was an issue with the validation algorithm with missing data.

-   [**Life events item coding error**](../release_notes/6_0.qmd#life-events-item-coding-error): The levels for the item `mh_y_ple__severity_032` were incorrectly coded as values 1-4 instead of 0-3 causing an inflation of values in all summary scores that item is included in.
...
![](/assets/img/documentation/deap_icon.svg){width="30"} `r link_table("mh_p_ders")`
`r r_link_table("mh_p_ders")`
...
![](/assets/img/documentation/deap_icon.svg){width="30"} `r link_table("mh_p_eatq")`
`r r_link_table("mh_p_eatq")`
...
Summary scores for some subscales (Activation, Aggression, Attention, Depressive Mood, Inhibitory Control, Shyness, and Surgency) of the `mh_p_eatq` measure were mistakenly reverse coded in the score algorithm despite already being reverse coded at the item level.
...
-   [**Life events summary score error**](../release_notes/6_0.qmd#life-events): The Life Events summary scores should not be used, because there was an issue with the validation algorithm with missing data.

-   [**Life events item coding error**](../release_notes/6_0.qmd#life-events-item-coding-error): The levels for the item `mh_p_ple__severity_032` were incorrectly coded as values 1-4 instead of 0-3 causing an inflation of values in all summary scores that item is included in.
