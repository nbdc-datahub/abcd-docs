...
<!-- [Download table as a pdf](../../assets/files/documentation/non_imaging/6_1_ab.pdf){style="display: block; text-align: right; margin-top: 4px;"} -->

"ABCD (General)" provides standard variables, as well as demographic information and surveys. In this new domain we introduce two new tables: `ab_g_dyn` and `ab_g_stc`. These tables use a new source `g` (for General), introduced specifically for these summary and administrative fields. The data in these tables are often derived or computed from multiple sources, including REDCap timestamps, research assistant (RA) input, and participant responses (youth and parent). The variables in these tables are cohort descriptors & useful nesting variables–participant demographic information, visit characteristics (site, date, etc), relational variables (family ID, principal genetic ancestry), etc–that are useful for cohort characterization and modeling. We anticipate that these tables will be of great importance for most research questions using ABCD data. 

# Parent tables

## Demographics {#ab_p_demo}

`r table_header("ab_p_demo")`

::: {.callout-caution collapse="true" title="Release notes: `ab_p_demo`"}
**7.0**

-   [**New item**](../release_notes/7_0.qmd#demographics): There were NAA mean scores added in the `ab_p_demo` where the Native American Acculturation items were asked to parents, the new item is `ab_p_demo__ntvam_mean`
:::

::: {.callout-important collapse="true" title="Responsible use consideration: Household income" #use-income}
...
Researchers are encouraged to explore how these response types relate to other variables and consider potential impacts on findings. Not accounting for these responses appropriately could introduce bias and compromise validity of results.
...
::: {.callout-warning collapse="true" title="Data consideration: Household income" #data-income}
...
**Key reference:** `r full_cite("li2025")`

:::
## Occupation Survey {#ab_p_ocp data-link="Occupation Survey"}

`r table_header("ab_p_ocp")`
...
The inclusion/exclusion criteria can be found [here](../../study/index.qmd#inclusionexclusion-criteria).
...
::: {.callout-warning collapse="true" title="Data consideration: `ab_g_dyn__visit_age` outliers"}
There are 4 participants who are more than 0.1 less than 9, or over 11 years old in the baseline event (`ab_g_dyn__visit_age`). Users can find these participants' ID in the private release notes and decide how to incorporate these data. The inclusion/exclusion criteria can be found [here](../../study/index.qmd#inclusionexclusion-criteria).
:::
...
::: {.callout-warning collapse="false" title="Data consideration: Site numbers"}
The number of site labels across the study's history is 22, but not the number of active sites, which is 21. In particular, Site 22 discontinued after baseline and most of its participants were transferred, primarily to Site 21. There are different ways to handle these participants' data: keep all 22 levels for baseline-only analyses; use a time-invariant site (baseline or current) for random intercepts; recode to the currently enrolled site if using fixed-effect site dummies, etc.
:::
...
::: {.callout-caution collapse="true" title="Release notes: `ab_g_dyn`"}
**6.1**

-   [**Levels labelling error fixed**](../release_notes/6_1.qmd#household-income-scores): A levels labeling error was identified in summary scores in 6.0 for the items `ab_g_dyn__cohort_income__hhold__3lvl` and `ab_g_dyn__cohort_income__hhold__6lvl`. The levels table has been corrected in the 6.1 patch release.
-   [**Site transfers**](../release_notes/6_1.qmd#site-transfers): Site transfers were updated for 2 participants that had been missed in 6.0 Data Release.

**6.0**

-   [**New tables**](../release_notes/6_0.qmd#standard-variables-tables): The 6.0 release includes [two new tables](https://abcd-internal.deapscience.com/?hierarchyOrder=%5B%22study%22%2C%22domain%22%2C%22subDomain%22%2C%22source%22%2C%22metric%22%2C%22atlas%22%5D&hierarchy=%5B%5B%22Standard+Variables%22%2C%22General%22%5D%5D#/my-datasets/create-dataset) that contain important variables likely to be of interest for a wide range of analyses. These tables include static variables (`r link_table("ab_g_stc", dest = "docs")`) and dynamic/longitudinal variables (`r link_table("ab_g_dyn", dest = "docs")`) that are not specific to any particular domain.
-   [**Updated school IDs**](../release_notes/6_0.qmd#school-and-district-ids): For the 6.0 release, school and district IDs were amended. These changes are specific to the pseudo school ID `ab_g_dyn__design_id__district` and `ab_g_dyn__design_id__school` and do not impact the linked SEDA data.
-   [**Levels labelling error**](../release_notes/6_0.qmd#household-income-scores)
  
:::

::: {.callout-important collapse="true" title="Responsible use consideration: Household income summary scores" #use-cohort_income__hhold}
**`ab_g_dyn__cohort_income__hhold__3lvl`, `ab_g_dyn__cohort_income__hhold__6lvl`**

[See consideration related to input variables](#use-income)
:::

::: {.callout-warning collapse="true" title="Data consideration: Household income summary scores" #data-cohort_income__hhold}

**`ab_g_dyn__cohort_income__hhold__3lvl`, `ab_g_dyn__cohort_income__hhold__6lvl`**

[See consideration related to input variables](#data-income)

:::

::: {.callout-warning collapse="true" title="Data consideration: Marital/partner and labor force participation variable" #data-ab_g_dyn__cohort_prtnrshp__employ}
...
**Key reference:** `r full_cite("heeringa2020")`
...
::: {.callout-caution collapse="true" title="Release notes: `ab_g_stc`"}
**6.0** 

-   [**New tables**](../release_notes/6_0.qmd#standard-variables-tables): The 6.0 release includes [two new tables](https://abcd-internal.deapscience.com/?hierarchyOrder=%5B%22study%22%2C%22domain%22%2C%22subDomain%22%2C%22source%22%2C%22metric%22%2C%22atlas%22%5D&hierarchy=%5B%5B%22Standard+Variables%22%2C%22General%22%5D%5D#/my-datasets/create-dataset) that contain important variables likely to be of interest for a wide range of analyses. These tables include static variables (`r link_table("ab_g_stc", dest = "docs")`) and dynamic/longitudinal variables (`r link_table("ab_g_dyn", dest = "docs")`) that are not specific to any particular domain.
-   [**Corrected sibling relationships**](../release_notes/6_0.qmd#family-and-birth-ids): In the 6.0 release, we corrected a small number of errors in the `ab_g_stc__design_id__fam` and `ab_g_stc__design_id__birth` variables to more accurately reflect sibling relationships between participants.
-   [**Ethno-racial summary scores**](../release_notes/6_0.qmd#ethno-racial-identity): Several new ethno-racial identity summary score variables are available in the data release, capturing ethnicity and race based on baseline and longitudinal responses from youth and parents.
-   [**Anonomized birth-dates**](../release_notes/6_0.qmd#anonymized-date-of-birth): We changed the algorithm for how anonomized birth-dates are computed in the 6.0 release.
:::

::: {.callout-important collapse="true" title="Responsible use consideration: Race & ethnicity" #use-race_ethnicity}
...
For recommendations on use of race/ethnicity data, see [@cardenas-iniguez2024].
:::

::: {.callout-important collapse="true" title="Data use consideration: Family relationship" #use-family_relationship}
...
::: {.callout-important collapse="true" title="Responsible use consideration: Genetics" #use-genetics}
