...
```{r}
render_table_info("domain_abbr == 'le'")
```

<!-- [Download table as a pdf](../../assets/files/documentation/non_imaging/6_1_le.pdf){style="display: block; text-align: right; margin-top: 4px;"} -->
...
::: {.callout-important collapse="true" title="Data use consideration: Linked External Data domain general" #use-le}
...
- `r full_cite("fan2021")`
- `r full_cite("cardenas-iniguez2024")`

# Stanford Education Data Archive (SEDA) {#SEDA}

National Center for Education Statistics (NCES) school and district IDs corresponding to the school/district the participant reported attending are linked to data from the SEDA Educational Opportunity Project 5.0 and the SEDA Education Recovery Project. To protect participant privacy, NCES school and district IDs and school names/addresses are not released; however, anonymous pseudo IDs are provided.

**SEDA Data Sources**

- **SEDA Educational Opportunity Project 5.0** includes academic achievement estimates from 2009–2019, allowing for examination of long-term educational opportunity and trends across U.S. districts and schools. More information can be found on the [SEDA website](https://edopportunity.org/opportunity/), and technical documentation can be found [here](https://stacks.stanford.edu/file/druid:cs829jn7849/SEDA_documentation_v5.0.pdf).
- **SEDA Education Recovery Project** includes annual district-level achievement data from the 2018–2019 through 2023–2024 academic years, enabling analyses of pandemic-related learning loss and recovery. These data support investigations of how factors such as remote learning and policy responses shaped student outcomes during and after the introduction of remote learning. More information can be found [here](https://edopportunity.org/recovery/).

**School history processing:**  
School history data were expanded to a monthly resolution. Temporal gaps were filled using forward-fill followed by backward-fill within each school year (defined as July through June), ensuring complete monthly coverage.

| Month   | School | Imputed |
|---------|--------|--------|
| July    | A      | A      |
| Aug     | NA     | A      |
| Sep     | NA     | A      |
...
SEDA data are provided in long format by year and subgroup. These data were reshaped to a wide format, resulting in one row per district or school with year- and subgroup-specific measures represented as separate columns (@reardon2025).

**Notes and special considerations:**  
Several data quality issues have been identified in the school history data. School attendance records may overlap temporally, resulting in multiple records per participant-month. This introduces ambiguity in assigning a single school or district, as well as in determining grade level and homeschool status for that period.

**Tabulated data:**  
Tabulated datasets are derived from file-based longitudinal data and reported at the visit level (`session_id`). 

Visit dates were aligned to monthly school records. For summer visits, school attendance was imputed using forward- and backward-fill.

Monthly school data were then linked to visit-level records. If multiple schools or districts were present for a given month, no data were assigned. The number of schools/districts is reported for each event, and the homeschool indicator reflects whether any record includes homeschooling-related terms.


::: {.callout-caution collapse="true" title="Release notes: SEDA Data"}
**7.0**

-   [**SEDA longitudinal data update**](../release_notes/7_0.qmd#seda-data): Beginning with the 7.0 data release, SEDA data are provided longitudinally across waves, rather than at baseline only, and are available in multiple formats (e.g., by assessment wave, school year, and month) to support different analytic approaches. Data linked by assessment wave can be downloaded within the tabulated datasets, whereas data linked by school year and month must be downloaded as [file-based data](#seda-file-based) through NBDC Data Access Platform.

-   [**SEDA pseudo-identifiers update**](../release_notes/7_0.qmd#seda-ids): pseudo-identifiers for this release were re-generated and should not be combined with pseudo-identifiers from previous releases.

`le_l_sedaadm__schl_id` - Anonymized school ID based on National Center for Education Statistics (NCES) school IDs [Cross-listed: `ab_g_dyn__design_id__school`]

`le_l_sedaadm__distr_id` - Anonymized school district ID based on National Center for Education Statistics (NCES) school IDs [Cross-listed: `ab_g_dyn__design_id__district`]

 **6.0**
 
- [**SEDA subtables**](../release_notes/6_0.qmd#school-information)
:::
::: {.callout-warning collapse="true" title="Data consideration: Stanford Education Data Archive (SEDA)" #data-seda}

Data disaggregated by race or ethnicity were intentionally not released.
...
ABCD has linked to the **cohort standardized (CS) scale scores**. The "CS" SEDA 5.0 files are recommended for research in SEDA documentation and include **Ordinary Least Squares** and **Empirical Bayes** estimation methods.
...
For all archived versions of SEDA visit [here](http://purl.stanford.edu/db586ns4974), @fahle2021. 
:::
### School Academic Achievement- Administrative (SEDA 5.0 & SEDA Recovery) {#le_l_sedaadm}
`r table_header("le_l_sedaadm")`
**Measure description:** Table includes pseudo district and school IDs, school grade, the number of schools and districts attended during the assessment period, and an indicator for homeschooling.

`le_l_sedaadm__distr_id` crosslisted as `ab_g_dyn__design_id__district` is the pseudo ID provided for clustering participants by school district
`le_l_sedaadm__schl_id` crosslisted as `ab_g_dyn__design_id__school` is the pseudo ID provided for clustering participants by school

School history data are collected using start month/year and end month/year, and include NCES school IDs corresponding to the reported school, when available.

Manually entered data are also captured to indicate homeschooling or related experiences. Homeschool records are identified when the reported school name includes terms such as homeschool, unschool, virtual, remote, cyber, or online. `le_l_sedaadm__homeschl_indicator` is provided as an indicator. 

### School Academic Achievement - District (SEDA 5.0) {#le_l_seda__distr}
`r table_header("le_l_seda__distr")`

**Measure description:** - Table includes SEDA 5.0 dataset average test scores (averaged across math and reading). The average test scores (math/reading) are included as a measure of educational opportunities at the district level. The measure also includes SEDA 5.0 dataset demographic covariates across years 2009-2019. 

The linked academic achievement variables have the following disaggregation: average test score mean (averaged across grades, years, and subjects), average “learning rate” across grades, average “trend” in the test scores across cohorts, and average difference between math and reading/ language arts (RLA) test scores, along with their standard errors. Estimates are reported for all students and by demographic subgroups.

The linked academic achievement variables have the following disaggregation: average test score mean in math and in RLA (averaged across grades and years), average “learning rate” across grades in math and in RLA, and average “trend” in the test scores across cohorts in math and in RLA, along with their standard errors.

**Notes and special considerations:** District-level SEDA data were linked to participant individual data in multiple ways: by assessment wave within the tabulated data (district ID at the time of the assessment), as well as school year (most common district ID each academic year), and monthly (most common district ID each month), which can be downloaded as file-based data in the NBDC Data Access Platform. Although SEDA data measures academic achievement for 3rd through 8th grade, we include district-level data regardless if participants are now in high school.

### School Academic Achievement - School (SEDA 5.0) {#le_l_seda-schl}

`r table_header("le_l_seda__schl")`

**Measure description:** Table includes SEDA 5.0 dataset average test scores (averaged across math and reading). The average test scores (math/reading) are included as a measure of math educational opportunities at the school level when participants are in 8th grade or below. The measure also includes SEDA 5.0 dataset demographic covariates across years 2009-2019.
...
**Notes and special considerations:** School-level SEDA data were linked to participant individual data in multiple ways: by assessment wave within the tabulated data (school ID at the time of the assessment), as well as academic year (most common school ID each academic year), and monthly (most common school ID each month), which can be downloaded through the NBDC Data Access Platform. School-level SEDA data is only available if the participant is enrolled in 3rd through 8th grade.

### School Academic Achievement Recovery - District (SEDA Recovery) {#le_l_sedarec__distr}

`r table_header("le_l_sedarec__distr")`
**Measure description:** Table includes SEDA Education Recovery Project dataset, which provides annual district-level achievement data from the 2018–2019 through 2023–2024 academic years. The table also provides pandemic-related learning loss and recovery scores, which are change scores between 2024 and 2019, 2024 and 2022, and 2022 and 2019. Positive scores indicate improved academic achievement across years, whereas negative scores indicate loss in academic achievement. 

# Residential history-derived environmental measures {#residential}

::: {.callout-caution collapse="true" title="Release notes: `Residential history`"}
**6.0**

-   [**Address collection**](../release_notes/6_0.qmd#data-collection-process): The original address data collection processes in ABCD relied on a point-in-time capture of residential addresses rather than recording longitudinal residential history.
:::
...
`r table_header("le_l_no2")`

**Measure description:** Measures of nitrous dioxide (NO2) were computed for all participants' primary, secondary, and tertiary addresses, when available. Air pollution estimates used a hybrid spatio-temporal model that provides daily estimates of air pollution levels at 1km x 1km resolution (@di2020). Measures in the current release include mean, minimum, and maximum NO2 from 2016.

**Key reference:**  `r full_cite("di2020")`

### Satellite-based O3 Measures (Air Quality Data for Health-Related Applications) {#le_l_o3}

`r table_header("le_l_o3")`

**Measure description:** Measures of ozone (O3) were computed for all participants' primary, secondary, and tertiary addresses, when available. Air pollution estimates used a hybrid spatio-temporal model that provides daily estimates of air pollution levels at 1km x 1km resolution (@requia2020). Measures in the current release include mean, minimum, and maximum O3 from 2016.

::: {.callout-caution collapse="true" title="Release notes: `le_l_lo3`"}
**7.0**

- [**Correctly mapped variable names**](../release_notes/7_0.qmd#satellite-based-o3-measures):There was an error during curation of the `le_l_lo3` table for the 6.0 release. This naming issue has now been corrected for the 7.0 release.

**6.0**
 
- [**Incorrectly mapped variable names**](../release_notes/6_0.qmd#satellite-based-o3-measures)
:::

**Key reference:**  `r full_cite("requia2020")`

### Satellite-based Particulate Measures {#le_l_particulat}

`r table_header("le_l_particulat")`
...
- `r full_cite("di2020")`
- `r full_cite("fan2021")`
- `r full_cite("herting2025")`
- `r full_cite("requia2020")`
...
::: {.callout-warning collapse="true" title="Data consideration: NaNDA" #data-nanda}
...
- `r full_cite("melendez2020")`
- `r full_cite("morenoff2007")` [Appendix A: Supplemental Material]

### Parks (NaNDa) {#le_l_parks}

`r table_header("le_l_parks")`
...
**Key reference:** `r full_cite("clarke2020")`

### Religious/Civic Organizations (NaNDa) {#le_l_relciv}

`r table_header("le_l_relciv")`
...
**Key reference:** `r full_cite("usdj2014")`

::: {.callout-important collapse="true" title="Responsible use consideration: Crime" #use-crime}
...
**Key reference:** `r full_cite("epa2014")`

### Population Density (EPA) {#le_l_denspop}

`r table_header("le_l_denspop")`
...
**Key reference:** `r full_cite("nasa2025")`

### Vehicle Density (ACS) {#le_l_densveh}

`r table_header("le_l_densveh")`
...
**Key reference:** `r full_cite("abad2023")`

### Lead Risk (Vox) {#le_l_leadrisk}

`r table_header("le_l_leadrisk")`
...
**Key reference:** `r full_cite("vox2016")`

::: {.callout-important collapse="true" title="Responsible use consideration: Lead risk" #use-lead}
...
- `r full_cite("lei2022")`
- `r full_cite("esri2025")`

### Traffic Density (Kalibrate) {#le_l_traffic}

`r table_header("le_l_traffic")`
...
**Key reference:** `r full_cite("ucb2010")`

### Walkability (EPA) {#le_l_walk}

`r table_header("le_l_walk")`
...
- LeqNight is the average sound level in from the hours of 10p-7a; and
...
- `r full_cite("mennitt2013")`
- `r full_cite("mennitt2016")`

### National Air Toxics Assessment (NATA) Measures {#le_l_nata}

`r table_header("le_l_nata")`
...
**Key reference:** `r full_cite("spangler2019")`

### Vapor Pressure Deficit (VPD) Estimates (PRISM) {#le_l_vpd}

`r table_header("le_l_vpd")`
...
- `r full_cite("schertz2022")`
- `r full_cite("mlrc2025")`
- `r full_cite("ucb2010a")`

## Neighborhood Composite Measures

### Area Deprivation Index (ADI) {#le_l_adi}

`r table_header("le_l_adi")`
...
**Key reference:** `r full_cite("kind2014")`

::: {.callout-important collapse="true" title="Responsible use consideration: Area deprivation index" #use-adi}
...
**Key reference:** `r full_cite("coi2023")`

::: {.callout-important collapse="true" title="Responsible use consideration: Child opportunity index" #use-coi}
Users should carefully consider the component variables and constructs of any composite measure, and choose the most appropriate measure(s) for their research questions. Composite measures may have overlapping variables, and researchers should consider issues of collinearity when using multiple composite measures. Additionally, certain indices—such as the ADI—may have documented issues with skewness and overrepresentation of particular component variables that may not be documented here. Additional responsible use considerations can be found [here](https://pubmed.ncbi.nlm.nih.gov/38195369/). 
:::

### Social Vulnerability Index (SVI) {#le_l_svi}

`r table_header("le_l_svi")`
...
::: {.callout-important collapse="true" title="Responsible use consideration: Social vulnerability index" #use-svi}
Users should carefully consider the component variables and constructs of any composite measure, and choose the most appropriate measure(s) for their research questions. Composite measures may have overlapping variables, and researchers should consider issues of collinearity when using multiple composite measures. Additionally, certain indices—such as the ADI—may have documented issues with skewness and overrepresentation of particular component variables that may not be documented here. Additional responsible use considerations can be found [here](https://pubmed.ncbi.nlm.nih.gov/38195369/). 
:::

### Supplemental measures to the Social Vulnerability Index (SSVI) {#le_l_ssvi}

`r table_header("le_l_ssvi")`
...
**Key reference:** `r full_cite("matthews2011")`

### Census Return {#le_l_censusret}

`r table_header("le_l_censusret")`
...
**Key reference:** `r full_cite("ucb2025")`

### Opportunity Zones and Investment Scores {#le_l_oz}

`r table_header("le_l_oz")`
...
**Key reference:** `r full_cite("theodos2018")`

### Rent and Mortgage Statistics {#le_l_rentmort}

`r table_header("le_l_rentmort")`
...
**Key reference:** `r full_cite("krieger2018")`

### Multi-group Entropy Index {#le_l_entropy}

`r table_header("le_l_entropy")`
...
**Key reference:** `r full_cite("iceland2004")`

## Policy

### Affordable Care Act Medicaid Expansion Data (KFF) {#le_l_aca}

`r table_header("le_l_aca")`
...
- `r full_cite("rand2025")`
- `r full_cite("haffajee2020")`


### OPTIC-Vetted Good Samaritan Policy Data (Optic) {#le_l_goodsam}

`r table_header("le_l_goodsam")`
...
- `r full_cite("davis2015")`
- `r full_cite("davis2017")`

### OPTIC-Vetted Medical Marijuana Policy Data (Optic) {#le_l_medmj}

`r table_header("le_l_medmj")`
...
- `r full_cite("pacula2014")`
- `r full_cite("ncsl2025")`
- `r full_cite("pacula2015")`
- `r full_cite("powell2018")`

### OPTIC-Vetted Naloxone Policy Data (Optic) {#le_l_polnalox}

`r table_header("le_l_polnalox")`
...
- `r full_cite("horwitz2018")`
- `r full_cite("pew2018")`
- `r full_cite("bja2025")`

# File-based data

## SEDA {#seda-file-based}
...
└── concatenated
    └── linked_external
        └── seda
            ├── seda_mo.<csv|parquet>
            ├── seda_schlyr.<csv|parquet>
            ├── sedarec_mo.<csv|parquet>
            └── sedarec_schlyr.<csv|parquet>
```            

### File details

Four file-based datasets are provided in csv and parquet format:

- `seda_mo`: School history by month linked to SEDA data
- `sedarec_mo`: School history by month linked to SEDA Recovery data
- `seda_schlyr`: School history by school year linked to SEDA data
- `sedarec_schlyr`: School history by school year linked to SEDA Recovery data

To download these data, follow the instructions on the ['Access & Download' page](../../usage/access.qmd).

::: {.callout-note title="File formats"}
The SEDA file-based data are made available as both `csv` and `parquet` files to support a range of tools and user preferences. However, since the `parquet` format ensures that data is imported with correctly specified data types and facilitates faster loading speeds, we recommend using `parquet` files over `csv` files whenever possible (see [here](../curation/structure.qmd#file-formats)).
:::

### Overview

SEDA-derived data are released as file-based data in both monthly and school-year formats, linked to either SEDA 5.0 or SEDA Recovery data. These file-based releases complement the tabulated SEDA tables described in the [SEDA section](#SEDA), which link SEDA measures to ABCD assessment waves; the file-based datasets retain the full longitudinal resolution (monthly or by school year) for analyses that require finer temporal granularity.

### Linkage and derivation

#### Monthly files

Monthly longitudinal data (`seda_mo`, `sedarec_mo`) were linked to SEDA data using NCES school IDs. Each participant-month was assigned SEDA measures based on the reported school or district. A homeschool indicator reflects whether the reported school name includes terms such as homeschool, unschool, virtual, remote, cyber, or online. School grade is also provided at the month level.

#### School-year files

To derive school-year data (`seda_schlyr`, `sedarec_schlyr`), monthly data were restricted to September through May, with each September–May period defined as a single school year to avoid summer gaps. For each school year, the school or district in which the participant spent the greatest number of months was selected. If multiple schools tied, no values were assigned. The number of schools/districts is reported for each school year, and the homeschool indicator reflects whether any school record included homeschooling terms.
