...
<!-- [Download table as a pdf](../../assets/files/documentation/non_imaging/6_1_nc.pdf){style="display: block; text-align: right; margin-top: 4px;"} -->
...
- `r full_cite("luciana2018")`
- `r full_cite("anokhin2022")`
...
- Picture Vocabulary Test and Picture Sequence Memory tests were administered remotely, but when done remotely, composite scores cannot be computed. Check `nc_y_nihtb_adm__rmt` to determine whether the task was administered in person or remotely.
...
::: {.callout-caution collapse="true" title="Release notes: `nc`"}
**6.0**

- [**Administrative variables**](../release_notes/6_0.qmd#neurocognition): In the 6.0 data release we removed the neurocognition administration table and added all relevant variables specific to a task’s administration to the tables themselves.
:::

::: {#use-nc .callout-important collapse="true" title="Responsible use consideration: Neurocognition domain general"}
...
- `r full_cite("wulfert2002")`
- `r full_cite("anokhin2011")`

## Delay Discounting {#nc_y_ddis}

`r table_header("nc_y_ddis")`
...
Users should consider restricting data analysis to participants for whom `values.Consistent per_JBcriterion1` and `values.Consistent per_JBcriterion2` are both "yes." The remote Millisecond version can be found [here](https://www.millisecond.com/library/delaydiscountingtask/abcddelaydiscounting).

**Key reference:** `r full_cite("johnson2008")`

## Emotional Stroop Task {#nc_y_est}

`r table_header("nc_y_est")`
...
**Notes and special considerations:** The intention was for participants to be assigned a stable button randomization (left-negative; right-negative). Due to RA error and intermittent technical issues some participants may have completed tasks using differing button randomizations across sessions. Data users are encouraged to utilize the variable `nc_y_est_keyassign` to see what task randomization was completed for the task and to account for this if stable assignment is needed for their analyses.

::: {.callout-caution collapse="true" title="Release notes: `nc_y_est`"}
**6.1** 

-   [**Reaction time**](../release_notes/6_1.qmd#emotional-stroop-task): 
...
::: {#data-est .callout-warning collapse="false" title="Data use consideration: Reaction Time"}
...
- `r full_cite("banich2019")`
- `r full_cite("basgoze2015")`
- `r full_cite("kane2003")`
- `r full_cite("stroop1935")`

## Millisecond Flanker Task {#nc_y_flnkr}

`r table_header("nc_y_flnkr")`

**Measure description:** This task measures attention, cognitive control, executive function, and inhibition of automatic response similarly to the NIH Toolbox Flanker task of the [NIH Toolbox (Cognition)](#nc_y_nihtb) battery. Starting at ses-02A, the Millisecond task was used for remote administration of the Flanker task ([smart phone version](https://www.millisecond.com/library/v7/flankertask/abcdarrowflankertest/abcdarrowflankertest_smartphone/abcdarrowflankertest_smartphone.manual), [tablet version](https://www.millisecond.com/library/v7/flankertask/abcdarrowflankertest/abcdarrowflankertest_tablet/abcdarrowflankertest_tablet.manual)), as the NIH Toolbox version could not be completed remotely. This task was designed to mimic the NIH Toolbox Flanker task as closely as possible. 

**Notes and special considerations:** We recommend that users carefully consider the administration differences between the NIH Toolbox Flanker task and the Millisecond Flanker task in their analyses. In addition, the two tasks were scored differently: NIH Toolbox computed a composite performance score based on accuracy and reaction time using a built-in algorithm (see the NIH Toolbox documentation), while Millisecond provided separate accuracy and reaction time measures. Therefore, the data from the two tasks cannot be combined, so users should plan accordingly when working with participant samples that include data from both tasks. Please note that some (but not all) participants were administered both Flanker tasks in the same follow-up years, while others were administered one of the tasks only.

## Game of Dice Task {#nc_y_gdt}

`r table_header("nc_y_gdt")`
...
- `r full_cite("brand2005")`
- `r full_cite("drechsler2008")`
- `r full_cite("duperrouzel2019")`
- `r full_cite("ross2016")`

## Little Man Task {#nc_y_lmt}

`r table_header("nc_y_lmt")`
...
**Modifications since initial administration**: The Little Man Task used in the baseline assessment was administered using a customized program designed by ABCD, whereas the 2-year and 4-year follow-up assessments used a [task](https://www.millisecond.com/library/mentalrotation/abcdlittlemantest) presented in the *Inquisit* system from [*Millisecond*](https://www.millisecond.com). We recommend users consider this difference in analyses.

::: {.callout-caution collapse="true" title="Release notes: `nc_y_lmt`"}
**7.0** 
-   [**Little Man Task**](../release_notes/7_0.qmd#little-man-task): Previously, some data were incorrectly entered in percentage form and some data contained zero, which might seem implausible. These have been fixed in 7.0.

**6.0**
-   [**Data errors**](../release_notes/6_0.qmd#neurocoginition-known-issues) 
:::

**Key reference:** `r full_cite("acker1982")`


## NIH Toolbox (Cognition) {#nc_y_nihtb}

`r table_header("nc_y_nihtb")`
...
**Modifications since initial administration**: Remote assessments in the starting at ses-02A completed a Flanker task using the *Inquisit* system from [Millisecond](https://www.millisecond.com/). This task was designed to mimic the NIH Toolbox Flanker task as closely as possible. We encourage users to consider this change in their analyses. 
...
::: {.callout-caution collapse="true" title="Release notes: `nc_y_nihtb`"}
**7.0** 

-   [**Missing file-based data**](../release_notes/7_0.qmd#nih-toolbox): During the generation of the file-based data for neurocognitive tasks, there was an issue that caused some file-based data not to be generated. This issue primarily affected the `ses-06A` timepoint and NIH Toolbox tasks.

**6.1**

-   [**Variables removed**](../release_notes/6_1.qmd#nih-toolbox): The `dtt` and `age` variables were removed for the `nc_y_nihtb__comp` subscales as those scales are not their own task, but rather combinations of other tasks in the NIH Toolbox and thus do not have their own unique time of administration.

**6.0**

-   [**Data outliers**](../release_notes/6_0.qmd#neurocoginition-known-issues): There are some potential outliers in the `nc_y_nihtb__readr__uncor_score` data.
:::

**Key reference:** `r full_cite("mcdonald2014")`

## The Pearson Rey Auditory Verbal Learning Test {#nc_y_ravlt}

`r table_header("nc_y_ravlt")`
...
**Modifications since initial administration**: An alternate form of the RAVLT was used in the 2-year follow-up, using words designed to be similar to those in the original list in difficulty, complexity, and length. The version administered at the 6-year follow-up is the same as the baseline version.

**Key references:**

- `r full_cite("strauss2006")`
- `r full_cite("lezak2012")`

## Social Influence Task {#nc_y_sit}

`r table_header("nc_y_sit")`
...
**Key reference:** `r full_cite("wechsler2014")`
...
**Key reference:** `r full_cite("snellen1862")`

## Edinburgh Handedness Inventory {#nc_y_ehis}

`r table_header("nc_y_ehis")`
...
- `r full_cite("oldfield1971")`
- `r full_cite("veale2014")`
...
The description of the Wechsler Intelligence Scale for Children - Matrix Reasoning is [here](#nc_y_wisc).
...
- `r full_cite("barkley2012")`
- `r full_cite("obrien2021")`
