...
::: {.callout-note collapse="true" title="Participant ID"}
`participant_id` is a randomized string of letters and numbers originally defined by the NIMH Data Archive. All participant identifiers start with the string `ses-` followed by a string of 8 letters and numbers. `participant_id` is not associated with any identifiable participant characteristics. 
:::

::: {#event-id .callout-note collapse="true" title="List of recurated session/event identifiers"}
The table below shows the recurated session identifiers and their corresponding labels for data included in the ABCD data release.

| Study     | Session/event ID | Session/event label        | Past release event ID      |
|-----------|------------------|----------------------------|----------------------------|
| Core      | ses-00S          | Screener                   | screener_arm_1             |
| Core      | ses-00A          | Baseline                   | baseline_year_1_arm_1      |
| Core      | ses-00M          | 0.5 Year                   | 6_month_follow_up_arm_1    |
| Core      | ses-01A          | 1 Year                     | 1_year_follow_up_y_arm_1   |
| Core      | ses-01M          | 1.5 Year                   | 18_month_follow_up_arm_1   |
| Core      | ses-02A          | 2 Year                     | 2_year_follow_up_y_arm_1   |
| Core      | ses-02M          | 2.5 Year                   | 30_month_follow_up_arm_1   |
| Core      | ses-03A          | 3 Year                     | 3_year_follow_up_y_arm_1   |
| Core      | ses-03M          | 3.5 Year                   | 42_month_follow_up_arm_1   |
| Core      | ses-04A          | 4 Year                     | 4_year_follow_up_y_arm_1   |
| Substudy  | ses-C01          | COVID Wave 1               | covid19_cv1_arm_2          |
| Substudy  | ses-C02          | COVID Wave 2               | covid19_cv2_arm_2          |
| Substudy  | ses-C03          | COVID Wave 3               | covid19_cv3_arm_2          |
| Substudy  | ses-C04          | COVID Wave 4               | covid19_cv4_arm_2          |
| Substudy  | ses-C05          | COVID Wave 5               | covid19_cv5_arm_2          |
| Substudy  | ses-C06          | COVID Wave 6               | covid19_cv6_arm_2          |
| Substudy  | ses-C07          | COVID Wave 7               | covid19_cv7_arm_2          |
| Substudy  | ses-S01          | SDev Wave 1                | sd_wave1_arm_3             |
| Substudy  | ses-S02          | SDev Wave 2                | sd_wave2_arm_3             |
| Substudy  | ses-S03          | SDev Wave 3                | sd_wave3_arm_3             |
:::
...
::: {.callout-important collapse="false" title="Issue with time stamps and ages"}
Please be aware of the following issues with the time stamps and ages reported for each table:

1. Timestamps were not always collected for all measures and tables, especially in earlier events. In those cases, `{table_name}_dtt` has a missing value and `{table_name}_age` is computed using the time stamp indicating the start of the event, `ab_g_dyn__visit_dtt`.
...
The code for all summary scores computed by the DAIRC^[Other summary scores, such as proprietary scores or summary scores imported from external sources, are not included in the `ABCDscores` R package.] is available in the `ABCDscores` R package on [GitHub](https://github.com/nbdc-datahub/ABCDscores), along with accompanying online [documentation](https://software.nbdc-datahub.org/ABCDscores). The R package aims to support transparency and reproducibility of ABCD release data by providing the exact algorithms and code used to compute the released summary scores, allowing users to tie a specific data release version to a corresponding version of the codebase (see also [here](../../tools/packages.qmd#ABCDscores) for the rationale behind creating this R package).
...
- **time**: Time of day formatted as a character string `"HH:MM:SS"`, representing a time without a date component; often denoted by a variable name ending in `_t`.^[The [`NBDCtools`](https://software.nbdc-datahub.org/NBDCtools) R package allows users to convert times to `HMS` format with the [`create_dataset()`](https://software.nbdc-datahub.org/NBDCtools/reference/create_dataset.html) and [`transf_time_to_hms()`](https://software.nbdc-datahub.org/NBDCtools/reference/transf_time_to_hms.html) functions.]
...
- Summary score items follow a standardized label format of `Table Name [Respondant] (Subscale): Score type [Validation:]` with additional, score specific information added when applicable. 
...
- `Peer Network Health [Youth]: Sum [Validation: No more than 0 missing or declined]`
