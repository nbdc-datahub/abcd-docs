...
[*Score subitems:*]{.underline} A *three-digit, zero-padded number*, e.g., `__002`, is used to indicate a summary score or derived item's relationship to the main item(s) used in the calculation. This is used in forms where scores are repeated over multiple cases of similar information so that each repeating score can be mapped back to the original content questions (e.g. `mh_p_famhx__alc__mat__unc_indicator__002`, *"Family History [Parent] (Alcohol) Endorsed: Maternal uncle 2"*, is the alcohol endorsement summary score for the youth's second reported maternal uncle and synthesizes data from all items beginning with `mh_p_famhx__alc__mat__unc_002` in the table.).
...
# Stata naming convention

Stata has a character limit that is not compatible with the current naming convention. As such, we added an additional name that is used when people are exporting data from DEAP and choose Stata as the output format. We created `name_stata` by concatenating tablename and a number which we get from ordersort (which is the order of variables within a table). When that order changes between releases, `name_stata` will be different between releases. See [DEAP documentation](https://docs.deapscience.com/datasets/my_datasets.html#stat_spss) for more information. 
