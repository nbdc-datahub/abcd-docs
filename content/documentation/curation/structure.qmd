...
ABCD's file-based data resource consists of raw and processed data files generated from various instruments and modalities, including magnetic resonance imaging (MRI), genetic analyses, wearable sensors, neurocognitive assessments, and other assessments. The resulting data are provided either as individual-level files (one file, or collection of files, per participant/event) or in a concatenated format (one file per instrument or modality, containing data for all participants/events).
...
Genetic, Fitbit, EARS data are released as file-based concatenated data. That is, they are not released through DEAP, which only releases the curated tabulated dataset (one row per participant/event) because these data have a different structure (time-series data by minute, day, or week). To download them, one needs to access [NBDC Data Access Platform](https://www.nbdc-datahub.org/) and download them using Globus like one would for the imaging data.

![](../../assets/img/documentation/concat.png)
:::
...
The `rawdata/` directory contains raw data from MR imaging, neurocognitive assessments, and wearable sensors, as well as a copy of the tabulated data resource and is organized as follows:
...
Additionally, if loading data in Python we recommend using `polars` as it offers an efficient memory model and query sequence making it faster than alternative packages. Errors have also been observed when reading `date` type data using `pandas`. 
:::
