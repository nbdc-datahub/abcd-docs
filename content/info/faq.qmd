...
Our data are publicly shared with eligible researchers with a valid research use of the data at a research institution. Please visit [How to Access Data](../usage/access.qmd) or the [NBDC Data Hub](https://www.nbdc-datahub.org/data-access-process) for more information on how to access and download the data.
:::
...
General information about the studies data dictionary & release variables can explored via DEAP and the NBDC Hub [(see details)](../tools/tools.qmd).
:::
...
::: {.callout-tip collapse="true" title="Is there any guidance regarding suppression of cell count in data reporting?"}
For ABCD study data, NIDA requires a threshold minimum n of 10 in public reporting of data, including scientific publications and presentations. Their recommendations can be found here. {{< downloadthis ../assets/files/NIDA_recommendations.docx dname="NIDA Recommendations" label="NIDA_recommendations.docx" type="light" >}}
:::

# Non-Imaging Data

::: {.callout-tip collapse="true" title="Where can I access Genetics, EARS, Fitbit data?"}

Genetics, Fitbit, EARS data are released as file-based concatenated data. That is, they are not released through DEAP, which only releases the curated tabulated dataset (one row per participant/event) because these data have a different structure (time-series data by minute, day, or week). To download them, one needs to access [NBDC Data Access Platform](https://www.nbdc-datahub.org/) and download them using Globus like one would for the imaging data. See [Data structure](../documentation/curation/structure.qmd) for more info.
...
There is currently no script available to run the ABCD minimal processing. There is a Docker container and Singularity image that runs the complete processing and analysis pipeline that is available here: <https://www.nitrc.org/projects/cmig_pipeline>.
:::
...
::: {.callout-tip collapse="true" title="Does processing for minimally processed fMRI data include slice timing correction?"}
No, the TR is relatively short so slice timing correction is unlikely to matter in the context of the slow hemodynamic response. The information about the slice timing for different platforms is in the attached [document](../assets/files/documentation/faq/ABCD_slice_timing.docx).
:::
